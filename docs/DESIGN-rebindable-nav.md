...
|---|---|---|---|---|
| 1 | `Ctrl+h/j/k/l` (cross-panel focus) | `plugin/input.rs:84-135`; scancode→direction at `navigation/window.rs:27-39` | `window::handle_window_nav`, `window.rs:74-144` | `input()` |
| 2 | `h/j/k/l` (intra-dock) | `navigation/dock.rs:93-108` (`resolve_dock_key`) (identity), `:87-122` (dispatch) | `dock_nav::handle_navigation` `:96-125`, `handle_hierarchy` `:127-136` | `input()` |
| 3 | `/`, `Enter`, `Esc` (intra-dock) | `navigation/dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms) | `handle_slash` `:162-171`, `handle_enter` `:178-202`, `handle_escape_from_dock` `:217-243` | `input()` |
| 4 | `Esc`, `Enter` (dock filter box) | `navigation/dock.rs:225-244` | same function; falls back to `handle_escape_from_dock()` at `:153` | `input()` |
| 5 | `a`, `d`, `r`, `y`, `R` (FileSystem) | `navigation/filesystem_explorer.rs:396-406` (`resolve_fs_action`); identity at `:363-378` | `begin_create` `:125-134`, `begin_delete`, `begin_rename`, `yank_path` `:109-115`, `refresh` `:117-123` | `input()` |
...
The real knot is that **five independent decisions are fused into a single match arm**, and the fusion is what makes the keyset unaddressable. `src/navigation/dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms):
...
- **Key identity is re-derived per site, three times, incompatibly.** `dock_hjkl` (`dock.rs:71-76`), `direction_from_hjkl` (`window.rs:23-29`) and `resolve_key` (`filesystem_explorer.rs:381-390` (`resolve_key`)) each implement "logical keycode first, physical fallback" over a different key subset. The consequence is a live bug: for a keyboard layout where the QWERTY-J position emits a logical `/`, `dock_hjkl` matches on the *physical* fallback and returns at `dock.rs:147-180`, so the `Key::SLASH => handle_slash` arm at `:127` is never reached. `/` is unreachable, silently, on exactly the layouts the fallback was added to support.
...
| C5 | **FileSystem-first ordering.** In the FS dock, `a/d/r/y/R` get first refusal; if the FS handler declines, generic dock handling still runs | `plugin/input.rs:140-150` | Specificity must be a declared, total, machine-checkable relation. Godot's scene depth **inverts** it: `dock` is derived from the focus owner, `dock.filesystem` from an ancestor via `is_ancestor_of` (`filesystem_explorer.rs:411-417`) |
...
| C9 | Focus changes are **deferred** — immediate `grab_focus()` during input processing is swallowed by Godot's dispatch loop | `dock.rs:292-299`; also `window.rs:138`, `cycle.rs:99`, `filesystem_explorer.rs:270` | One helper, four verbatim call sites preserved |
...
The one genuine one-way trap is elsewhere and is **not** fixed here: `handle_escape_from_dock` (`dock.rs:319-330`) falls back to focusing a plain `TextEdit`, which is unconditionally `Foreign` (`focus.rs:85-87`) and which `find_code_edit_from_control` can never attach to — so there is no auto-attach rescue and no keyboard escape. It requires a `ScriptEditorBase` exposing a `TextEdit` but no `CodeEdit`, which stock Godot 4 does not produce, so it is carried as a low-probability known issue, pre-registered in prose at §11.2 (it is unreachable headlessly until P4's `FocusChain` seam exists).
...
```rust
// src/navigation/dock.rs:111-118
...
|---|---|---|---|
| 1 | **which key** | `hjkl_to_dock`, `dock.rs:111-118` | a `vim_core::keymap::MappingTrie` the user owns, one per surface — key → *name* |
| 2 | **which widgets are eligible** | `matches!(dock_kind, DockKind::Tree)`, `dock.rs:163,112` | `ActionSpec::requires: Caps`, a subset test evaluated in `resolve.rs` |
...
| `DockKind::Tree` gating `h`/`l` | `matches!(dock_kind, DockKind::Tree) && handle_hierarchy(…)`, `dock.rs:163,112` | the binding requires `Caps::HIERARCHY`; an `ItemList` does not contribute it; the candidate is skipped and the walk continues. No widget name in the dispatcher |
| `j` at the end of a list | `if handle_navigation(…) { Handled } else { Ignored }`, `dock.rs:149-154` | the action returns `Declined`; the walk continues; nothing else matches; nothing is consumed. Godot's `Tree` sees the key |
...
bitflags::bitflags! {
    ...
    pub(crate) struct Caps: u16 {
        ...
        /// `DockKind::Tree` gates at dock.rs:163,113 are already redundant.
        const HIERARCHY = 1 << 1;
        /// Activate selection. Tree + ItemList; reproduces
        /// `DockKind::RichTextLabel => Ignored` at dock.rs:288.
        const ACTIVATE  = 1 << 2;
...
pub(crate) struct FocusChain {
    ...
    /// src/navigation/filesystem_explorer.rs:413-417.
...
bitflags::bitflags! {
    ...
    pub(crate) struct Caps: u16 {
        ...
        /// Expand/collapse. Tree only: `handle_hierarchy` returns false for
        /// every other class (dock_nav.rs:127-136), which is why the
        /// `DockKind::Tree` gates at dock.rs:163,112 are already redundant.
        const HIERARCHY = 1 << 1;
        /// Activate selection. Tree + ItemList. Reproduces the
        /// `DockKind::RichTextLabel => Ignored` arm at dock.rs:288.
        const ACTIVATE  = 1 << 2;
        /// Focus owner is a text input (LineEdit). Reproduces focus.rs:73-82.
        const TEXTENTRY = 1 << 3;
...
Three flags from the original vocabulary are **deleted**: `PANEL` (granted by the forest root to every non-barrier surface, so `requires: PANEL` is a tautology — `godotvim.focus.*` take `Caps::empty()`), `ESCAPE` (has no possible grantor; `handle_escape_from_dock` already declines when the script editor is missing, `dock.rs:307-309`), and `SEARCHBOX` (redundant with `handle_slash`'s own decline when `find_sibling_search_box` returns `None`, `dock.rs:256-258`). Deleting `SEARCHBOX` is what removes the depth-8-climb × depth-20-DFS (`dock_search.rs:37-58`) from the per-focus-change path entirely — `handle_slash` runs it itself, once per `/` press, exactly as today.

`FILEOPS` is the one non-tautological surface-granted cap and must stay: without it, `panelmap dock a godotvim.fs.create` would create files at `res://` root when focus is on a Scene tree, because `get_selected_path` returns `None` for a non-FS Tree and `target_dir` falls back to `"res://"` (`filesystem_explorer.rs:129-133`).
...
pub(crate) struct FocusChain {
    ...
    /// unbounded, matching src/navigation/filesystem_explorer.rs:411-417.
    /// Grants Caps::FILEOPS.
    pub(crate) in_filesystem_dock: bool,
    ...
    /// (filesystem_explorer.rs:103-110). Probed BEFORE `searchbox` AND BEFORE
...
impl ActionCtx<'_> {
    ...
    /// copies at dock.rs:298, window.rs:138, cycle.rs:99,
    /// filesystem_explorer.rs:270 — all four are `.call_deferred("grab_focus", &[])`.
    ...
    /// clone-and-inject path at filesystem_explorer.rs:468-503, behind the
...
* `handle_dock_input` rejects **all** modifiers including shift (`dock.rs:133-137` (the `any_modifier` guard));
* `handle_search_input` rejects ctrl/alt/meta but **not** shift (`dock.rs:212` (`resolve_search_key`));
* `FileSystemExplorer::handle_key` uses shift as a **discriminant**: `match (key, shift)` with `(Some(Key::R), true) => refresh` (`filesystem_explorer.rs:396-406` (`resolve_fs_action`)).
...
pub(crate) enum Consumption {
    ...
    /// (dock.rs:148-154) and Enter with nothing selected (dock.rs:240-243).
    Elastic,
...
`src/actions/keys.rs` replaces all three ad-hoc per-site fallbacks that exist today: `dock_hjkl`/`hjkl_to_dock` (`src/navigation/dock.rs:72-86`), `direction_from_hjkl`/`hjkl_direction` (`src/navigation/window.rs:27-38`), and `resolve_key`/`is_fs_key` (`src/navigation/filesystem_explorer.rs:381-390` (`resolve_key`)).
...
There is no fourth probe. The synthesis carried a "named key with SHIFT cleared" probe; it is deleted, because it is not a key *identity* question but a per-binding matching *tolerance*, and there are three regimes in today's source, not two: `handle_dock_input` rejects all modifiers including shift (`dock.rs:133-137` (the `any_modifier` guard)), `handle_search_input` rejects ctrl/alt/meta but tolerates shift (`dock.rs:212` (`resolve_search_key`)), and `FileSystemExplorer::handle_key` uses shift as a *discriminant* — `match (key, shift) { (Some(Key::R), true) => self.refresh(), … }` (`filesystem_explorer.rs:396-406` (`resolve_fs_action`)). A global stage cannot express that; a per-surface bool expresses only two of three. It is modelled instead as `Rule::shift_tolerant`, expanded at **registration** into a second trie LHS pointing at the same `SlotId`. Exactly two rules carry it (`searchbox <CR>` and `searchbox <Esc>`), and dispatch gains no stage.
...
Moving it here changes *when* it runs, and that change is an improvement to be pre-registered rather than avoided. Today it is unreachable for Ctrl+hjkl, because `should_intercept_hjkl` returns at `input.rs:133` before the `FocusContext::Dock` arm ever calls `handle_key`. So today, opening the FS create prompt (which grabs focus, `filesystem_explorer.rs:235`) and pressing Ctrl+L jumps focus to another panel while the prompt stays visible with its `active_control` still pointing at the old FileSystem Tree — and whenever `dismiss_prompt` eventually runs it will `call_deferred("grab_focus")` back to that stale Tree (`filesystem_explorer.rs:265-272`), stealing focus from wherever the user has moved. Running the hook at S4 dismisses the orphan. Everything else is preserved verbatim, including that it fires for *modified* keys — `handle_key` calls `validate_cache()` and the dismiss at `:74-80` **before** the ctrl/alt/meta filter at `:82`, so Alt+X in the FS dock already auto-dismisses today.
...
- **`Elastic`** (the default, and every non-`panel` rule) consumes iff the action accepted. This is what preserves the two tri-state returns the whole design rests on: `j` at the end of an `ItemList` (`dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms)) and Enter with nothing selected (`dock.rs:240-243`) are not consumed, so Godot's Tree type-to-search and arrows (`gui_input`) and the docks' F2/Delete accelerators (`shortcut_input`) still see the key.
...
4. **Deepest declared surface wins**, walking the forest path leaf→root: `dock.filesystem` > `dock` > `panel`. Depth is authored in `SurfaceSpec::parent`, validated at registration, printed by `:panelmap`. It is never scene-tree depth — Godot's focus chain is generic-at-the-leaf and specific-at-the-ancestor (`dock` comes from `gui_get_focus_owner()`, `dock.filesystem` from `FileSystemDock::is_ancestor_of`, `filesystem_explorer.rs:411-417`), so scene depth would invert FileSystem-first refusal.
...
`RuleTarget::Shortcut(path)` and the `godotvim.fs.{delete,rename}` executors both delegate to one of Godot's own registered editor shortcuts by cloning its `InputEventKey` and re-injecting it via `Input::parse_input_event` — the path proven at `src/navigation/filesystem_explorer.rs:468-503`. That re-enters the same pipeline that dispatched it.
...
| 3 | `j`, Script list `ItemList`, last item already selected | `[ItemList(VNAV\|ACT), …]` | `[dock, panel]` | p1 `j` → `dock j` → `item.next` | `Declined` (`handle_navigation` returns false, `dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms)) | **No** — walk exhausts; Godot's ItemList sees the key |
| 4 | `l`, same `ItemList` | same | same | p1 `l` → `dock l` → `item.expand` requires `HIERARCHY`; path caps are `VNAV\|ACT` → **gated out**, treated as `NoMatch`; `panel` has no `l` | — (never ran) | **No** — replaces the `DockKind::Tree` gate at `dock.rs:217` (`resolve_search_key`’s Escape/Enter arm) |
...
**A blanket physical-keycode fallback evaluated per match arm.** This is the live `/`-on-physical-J bug: the hjkl block at `src/navigation/dock.rs:139-183` (the `resolve_dock_key` dispatch) precedes the `Key::SLASH` arm at `:127`, so on a Colemak/Dvorak layout the logical `/` never reaches `handle_slash`. Generalized naively it fires `godotvim.fs.yank_path` when a QWERTZ user presses `z`. Replaced by exactly one ordered probe applied once to the whole key — as-typed after langmap, then `latin_key` normalization, then US-QWERTY position; there is no fourth "named key with SHIFT cleared" stage, because that is a per-binding tolerance (`Rule::shift_tolerant`) rather than a key identity (§5.3) — with the physical probe opt-in per rule, set on exactly the fourteen rules that have that fallback today (fourteen *rules*, not thirteen: `R`/refresh also routes through `resolve_key` at `src/navigation/filesystem_explorer.rs:396` (`resolve_fs_action`), feeding the `(Some(Key::R), true)` arm at `:95`).
...
**Work.** Build `src/testing/focus_fixture.rs`: `FocusChain` as literal data (class chain, instance ids, precomputed facts) with no `Gd<T>` anywhere, plus `src/testing/action_recorder.rs`, a `RecordingCtx` that logs `grab_focus` / `emit_signal` / `push_input` calls with their arguments instead of performing them. Then characterize *current* behaviour of all seven hardcoded sites: the Ctrl+hjkl direction table (`src/navigation/window.rs:23-39`), dock `j`/`k`/`h`/`l` including the `DockKind::Tree` gates (`src/navigation/dock.rs:139-183` (the `resolve_dock_key` dispatch)), `/`/Enter/Esc (`:124-132`), search-box Esc/Enter with its Shift tolerance (`:138-159`), FS `a`/`d`/`r`/`y` with the `(Some(Key::R), true)` refresh discriminant (`src/navigation/filesystem_explorer.rs:396-406` (`resolve_fs_action`)), the FS prompt's hardcoded `Key::ESCAPE` (`src/plugin/mod.rs:789-798`), and both tri-state `Declined` returns (`dock.rs:155-161`, `:195-198`).

**Files.** MOD `src/navigation/{window,dock,filesystem_explorer}.rs` (the five extractions above); NEW `#[cfg(test)] mod characterization` in `src/navigation/{window,dock,dock_nav,filesystem_explorer}.rs`.

**Deferred, with reasons.** `src/testing/focus_fixture.rs` moves to **P4** — it fixtures `FocusChain`, which P4 defines; P0 cannot fixture a type that does not exist yet (P2's own text already says so). `src/testing/action_recorder.rs` moves to **P2** — a `RecordingCtx` needs something to record through, and nothing takes a ctx until P2 introduces `ActionCtx`; today `handle_enter` calls `emit_signal` directly on a `Gd<Control>`. P2's gate depends on it, so it lands there. `cycle.rs` and `focus.rs` get no test module: every decision in them is behind `EditorInterface::singleton()`.

**Gate.** `cargo test` green. Known bugs are pinned at their **wrong** value as ordinary `known_bug_*` tests that run by default — NOT `#[ignore]`d. An ignored test is silent both while the bug exists and after it is fixed, so nothing ever forces the revisit; an inverted assertion goes red the moment the named phase fixes it, which is the whole point. It is also truer characterization: pin what the code does, not what it should do. Three ship (§11): the `/`-shadow, numpad Enter, and QWERTZ `z`. Reviewer check: no `Gd<T>` appears in any fixture type.

**Shippable / revertable.** Yes — no production behaviour changes. It is *not* tests-only, and cannot be: a `cdylib` GDExtension cannot construct `Gd<T>` under `cargo test`, so every decision reachable only through a `Gd<InputEventKey>` is unreachable by any test. Three behaviour-preserving extractions are therefore prerequisites, not scope creep:

- `window.rs` — the cone predicate to `fn in_cone(diff: Vector2, dir) -> bool`, and the incumbent-beating rule to `fn beats_incumbent(in_cone, dist, best) -> bool`.
- `dock.rs` — `dock_hjkl(&Gd<InputEventKey>)` to `fn resolve_dock_key(logical, physical, any_modifier) -> DockKeyAction`, and the search-box guard to `fn resolve_search_key(logical, ctrl_alt_or_meta) -> SearchKeyAction`.
- `filesystem_explorer.rs` — the `(key, shift)` match to `fn resolve_fs_action(logical, physical, shift) -> FsAction`.

Each is a verbatim move of the decision with the `Gd` accessors hoisted to the call site. Without them the `/`-shadowing and QWERTZ defects below could not be expressed as assertions at all.

### P1 — One key vocabulary, behind today's dispatcher (deps: P0)

**Work.** Widen `physical_to_ascii` (`src/bridge/input.rs:97`) and `normalize_key_for_mapping` (`src/controller/process.rs:541`) to `pub(crate)`. Add `src/actions/keys.rs` with `probes()`, `canonicalize()`, `validate_lhs_key()` and `parse_lhs()`. Plumb `LangmapTable::parse(controller.engine().options().langmap())` into a table rebuilt at the config-source sites — new plumbing; godot-vim uses langmap nowhere today. Call `parse_godot_key` once at the top of `handle_input_impl` and thread the probe list into the three existing handlers, deleting `dock_hjkl`/`hjkl_to_dock` (`dock.rs:93-108` (`resolve_dock_key`)), `direction_from_hjkl`/`hjkl_direction` (`window.rs:23-39`) and `resolve_key`/`is_fs_key` (`filesystem_explorer.rs:381-390` (`resolve_key`)).
...
**Gate.** P1 is the one phase licensed to edit P0's suite, and only in a bounded way: it deletes `hjkl_to_dock` / `hjkl_direction` / `is_fs_key` and rewrites the three resolvers, so the tests bound to those symbols move with them. What must NOT change is the asserted behaviour, except for the three `known_bug_*` tests, which P1 flips to their correct value and renames. Every other assertion carries over verbatim. (P2 and P6 are the phases that must pass the suite **completely** unmodified.) Two of the pinned known bugs turn green (the third, the QWERTZ alias, is not a bug — see §11.2): `/` on a physical-J layout now reaches `handle_slash`; numpad Enter works (`get_named_key` already maps `KP_ENTER → Key::Enter` at `src/bridge/input.rs:23`); Ctrl+hjkl resolves via the physical probe from the editor. Plus the layout matrix and the corrected normalization proptest (§11).
...
**Known bugs, pinned at their wrong value.** A refactor that fixes a bug it did not intend to fix is suspicious, so every intended fix is pre-registered as an assertion rather than discovered in review. These are **not** `#[ignore]`d: they run by default and are green today, asserting what the code actually does. The named phase must flip each one, and the test goes red until it does — which is what forces the acknowledgement. `#[ignore]` cannot do that, because an ignored test is equally silent before and after the fix.

| Known bug, pinned | Flipped by | Why it is wrong today |
|---|---|---|
| `resolve_dock_key(SLASH, J)` yields `Hjkl(Down)`, not `Slash` | P1 | the hjkl probe consults logical **and** physical before the SLASH arm is reached, so `/` is unreachable on exactly the layouts the fallback serves. Not `/`-specific: any logical key over a physical hjkl position is swallowed (`ESCAPE`/`J`, `ENTER`/`L` likewise) |
| `resolve_dock_key(KP_ENTER, KP_ENTER)` yields `None`, not `Enter` | P1 | the dock path matches the raw keycode; only the editor path folds `KP_ENTER → Key::Enter` via `get_named_key` (`src/bridge/input.rs:23`) |
| `resolve_fs_action` on a QWERTZ `z` yields `YankPath` | **never — by design** | The physical alias is the deliberate cost of supporting position-based layouts (Dvorak, Colemak, AZERTY), and §5.3 keeps it on exactly the fourteen rules that carry the `physical` flag — `dock.filesystem y` among them. P1 makes it *ordered* (probe 1 gets first refusal); P5 makes it *escapable* (`panelmap dock.filesystem z <target>` wins probe 1). An earlier draft of this row said P1 flips it to `None`, which contradicted §5.3 and briefly misled P0. |

Two further defects are pre-registered in prose rather than as tests, because neither is reachable headlessly:

- **Ctrl+hjkl does not navigate from the *editor* on a Cyrillic/Greek layout** (turns green at P1). The escape-hatch block at `src/plugin/input.rs:110-116` matches the logical keycode only and `return false`s before the physical fallback at `:126`. Testable once `should_intercept_hjkl`'s key probe is extracted; `input.rs` is not on P0's file list, so P1 extracts and pins it in the same commit that fixes it.
- **`handle_escape_from_dock`'s plain-`TextEdit` fallback is a one-way trap** (unfixed, low probability). It focuses a `TextEdit`, which is always `Foreign` (`focus.rs:85-87`) and can never auto-attach, so neither `Ctrl+hjkl` nor `Esc` escapes. Every path into it runs through `EditorInterface::singleton()`, so it is out of reach until P4's `FocusChain` seam exists.
...
| `dock` | `j` | `godotvim.item.next` | `<physical>` | `dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms); declines at end-of-list |
| `dock` | `k` | `godotvim.item.prev` | `<physical>` | `dock.rs:155-161` |
| `dock` | `h` | `godotvim.item.collapse` | `<physical>` | `dock.rs:163` — the `DockKind::Tree` gate becomes `requires: Caps::HIERARCHY` |
| `dock` | `l` | `godotvim.item.expand` | `<physical>` | `dock.rs:217` (`resolve_search_key`’s Escape/Enter arm) |
| `dock` | `/` | `godotvim.dock.search` | `<physical>` | `dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms), `:130` |
| `dock` | `<CR>` | `godotvim.item.activate` | — | `dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms); `requires: Caps::ACTIVATE` reproduces `RichTextLabel => Ignored` at `:200` |
| `dock` | `<Esc>` | `godotvim.focus.editor` | — | `dock.rs:99-107` (`resolve_dock_key`’s slash/enter/escape arms) |
| `dock.filesystem` | `a` | `godotvim.fs.create` | `<physical>` | `filesystem_explorer.rs:398` (`resolve_fs_action`’s Create arm) |
...
1. **`/` works on a physical-J layout.** Today the hjkl block at `dock.rs:139-183` (the `resolve_dock_key` dispatch) precedes the `Key::SLASH` arm at `:127`, so logical `/` is shadowed. The global probe order applies to the whole key, once, so it cannot recur.
