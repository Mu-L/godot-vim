...
One live defect the design inherits rather than fixes: `is_navigable_control` admits `CodeEdit` (`scene_tree.rs:30-36`) and `is_window_candidate` accepts an unrecognised control anyway (`window.rs:203-208`), so a *foreign* `CodeEdit` — another addon's editor — is a legal `Ctrl+hjkl` destination, while `classify_focus` maps it to `Foreign` (`focus.rs:56-57`) and `input.rs:90` refuses to intercept from it. You can navigate in and not out. C4 forbids weakening the hard stop, so the fix belongs in `is_window_candidate`, not in the dispatcher. It is carried as a known issue with a named test, not smoothed over.
...
**Behaviour removals users may notice.** Excluding non-attached CodeEdits from `is_navigable_control` removes a Ctrl+hjkl *target* that exists today (a foreign addon's code editor). Nobody can currently navigate *out* of it, so the net effect is "a panel you could get stuck in is no longer reachable" — but it is a removal, not purely a fix, and belongs in the migration notes. Separately, `godotvim.focus.cycle_next/prev` ship **unbound** by deliberate choice: no safe key was available (Ctrl+W and Ctrl+N are both taken by the editor), so discovery depends entirely on `:checkhealth` and a commented recipe in `.godot-vimrc.sample`.
