...
| `resolve_fs_action(Z, Y, false)` yields `YankPath`, not `None` | P1 | the physical fallback is unconditional, so a QWERTZ `z` — a perfectly good Latin key that simply is not bound — yanks the selected path |
